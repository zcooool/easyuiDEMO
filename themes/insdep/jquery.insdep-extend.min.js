	...
	$.insdep={
		...
		ajax:function(c){
			var d={
				...
				dataType:"html",
				beforeSend:function(){
					$.messager.progress();//ps：显示出加载层
				},
				complete:function(){
					$.messager.progress('close');//ps：这边是加载层的处理
				}
			};
		...
		//ps：弹出easyui的弹出框，如果可以，可以使用layer的弹出层。
		...
		//ps：用于处理#control层url显示的问题，这个功能用单页面的使，注意关于超时的处理，如果可以可以将这2该方法合成一个方法，
		// 通过layout_loading中设置相应的参数来实现多页面切换，还是但页面切换。注意判定一个control
		control:function(url,title,queryParams){
            if($.config.debug){
                console.log(new Date()+'访问了'+url);
